...
export function LiveSearch({ catalog, value, onChange, onSelectProduct }: LiveSearchProps) {
  ...
  return (
    <div className="relative ml-auto w-full min-w-0 flex-1 lg:ml-0">
      <label className="glass-field group flex h-11 items-center rounded-full py-1 pl-5 pr-1"><input value={value} onChange={(event) => applySearch(event.target.value)} onFocus={() => setFocused(true)} onBlur={() => window.setTimeout(() => setFocused(false), 140)} onKeyDown={onKeyDown} aria-label="Search NetLet" aria-expanded={showPanel} aria-controls="netlet-search-suggestions" className="type-body w-full bg-transparent px-3 outline-none placeholder:text-[#778ba6]" placeholder={`Search for ${idleExamples[idleIndex]}`} />{value ? <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => applySearch("")} aria-label="Clear search" className="glass type-control grid size-7 place-items-center rounded-full !text-[#536b8c]"><X className="size-4" /></button> : null}<button type="button" onMouseDown={(event) => event.preventDefault()} onClick={submitSearch} aria-label="Search the catalog" className="glass glass-navy pressable ml-1.5 grid size-9 shrink-0 place-items-center rounded-full"><Search className="size-[17px]" /></button></label>
