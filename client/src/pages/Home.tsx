...
const heroImage = "/manus-storage/soukora-hero-living_ef30008d.jpg";
const logoImage = "/manus-storage/netlet-logo-transparent_3d66ed60.png";
const footerLogoImage = "/manus-storage/netlet-footer-white-transparent_a243ae75.png";
...
function IconButton({ label, children, onClick, className = "" }: { label: string; children: React.ReactNode; onClick?: () => void; className?: string }) {
  return <button aria-label={label} title={label} onClick={onClick} className={`pressable grid size-10 place-items-center rounded-full text-[#0a285a] transition-colors hover:bg-[#e7edf5] ${className}`}>{children}</button>;
}
...
function CustomerQuickControls({ onDelivery }: { onDelivery: () => void }) {
  ...
  return <div className="ml-auto hidden w-max max-w-[calc(100vw-2rem)] shrink-0 items-center justify-end gap-2 lg:flex" dir="ltr">
    <button onClick={onDelivery} className="pressable flex max-w-[230px] items-center gap-2 whitespace-nowrap rounded-full border border-[#d5dfeb] bg-[#fffdf9]/95 px-3.5 py-2 text-xs font-bold text-[#0a285a] shadow-[0_8px_22px_rgba(10,40,90,.08)] backdrop-blur-xl hover:border-[#aac0da]"><MapPin className="size-3.5 shrink-0 text-[#f2683a]" /><span className="truncate">{labels.delivery} {deliveryZone ? (isArabic ? deliveryZone.labelAr : deliveryZone.label) : (isArabic ? "اختر منطقتك" : "select area")}</span></button>
    <button onClick={toggleLocale} className="pressable flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border border-[#d5dfeb] bg-[#fffdf9]/95 px-3.5 py-2 text-xs font-bold text-[#0a285a] shadow-[0_8px_22px_rgba(10,40,90,.08)] backdrop-blur-xl hover:border-[#aac0da]"><Globe2 className="size-3.5 text-[#f2683a]" />{labels.language}</button>
  </div>;
}
  ...
  return (
    <article className={`product-card group relative overflow-hidden rounded-2xl border border-[#d5dfeb] bg-[#fffdf9] ${compact ? "p-2.5" : "p-3"}`}>
      ...
      <div className="px-0.5 pb-0.5 pt-3">
        ...
        <div className="mt-3 flex items-end justify-between"><div><p className="type-price text-sm font-extrabold tracking-[-.04em] text-[#0a285a]">{formatMoney(product.priceRange.min)}</p>{compareAt && <p className="type-label text-[9px] text-[#778ba6] line-through">{formatMoney(compareAt)}</p>}</div><button disabled={!canBuy || isAdding} onClick={onAdd} className="type-control pressable grid size-8 place-items-center rounded-full bg-[#0a285a] text-white hover:bg-[#f2683a] disabled:cursor-not-allowed disabled:opacity-50" aria-label={canBuy ? `Add ${product.title} to bag` : `${product.title} is unavailable`}>{isAdding ? <LoaderCircle className="size-3.5 animate-spin" /> : <ShoppingBag className="size-3.5" />}</button></div>
      </div>
    </article>
  );
}
...
export default function Home() {
  ...
  return <main id="top" className="min-h-screen overflow-x-hidden bg-[#f3f2ed] text-[#0a285a]">
    ...
    <footer className="bg-[#0a285a] pb-24 pt-12 text-white lg:pb-7"><div className="container grid gap-9 sm:grid-cols-2 lg:grid-cols-[1.5fr_repeat(4,1fr)]"><div><FooterLogo /><p className="mt-4 max-w-xs text-sm leading-6 text-[#b9cce5]">A considered marketplace for everyday Kuwaiti life and the things that make it feel more yours.</p><button onClick={() => toast("Arabic storefront is planned for the next catalog phase.")} className="mt-6 rounded-full border border-[#49658d] px-3 py-2 text-[10px] font-extrabold tracking-[.08em] text-white uppercase hover:bg-white/10">العربية</button></div>{footerGroups.map((group) => <div key={group.title}><p className="text-xs font-extrabold text-white">{group.title}</p>{group.links.map((link) => <button key={link} onClick={() => link === "All products" ? showAll() : toast(`${link} is being prepared for the next storefront phase.`)} className="mt-3 block text-left text-xs font-medium text-[#b9cce5] hover:text-[#ffcc64]">{link}</button>)}</div>)}</div><div className="container mt-10 flex flex-col gap-3 border-t border-[#49658d] pt-5 text-[10px] font-semibold text-[#b9cce5] sm:flex-row sm:items-center sm:justify-between"><span>© 2026 NetLet. Kuwait marketplace preview.</span><span>Privacy · Terms · Contact</span></div></footer>

    <nav className="fixed inset-x-3 bottom-3 z-30 flex items-center justify-around rounded-2xl border border-[#d5dfeb] bg-[#fffdf9]/95 px-2 py-2 shadow-[0_12px_35px_rgba(10,40,90,.16)] backdrop-blur-xl lg:hidden" aria-label="Mobile navigation"><button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="pressable grid place-items-center gap-0.5 p-1.5 text-[10px] font-bold text-[#0a285a]"><Store className="size-5" />Home</button><button onClick={() => setMenuOpen(true)} className="pressable grid place-items-center gap-0.5 p-1.5 text-[10px] font-bold text-[#536b8c]"><Menu className="size-5" />Browse</button><button onClick={() => toast(`${saved.length} saved item${saved.length === 1 ? "" : "s"}.`)} className="pressable grid place-items-center gap-0.5 p-1.5 text-[10px] font-bold text-[#536b8c]"><Heart className="size-5" />Saved</button><button onClick={() => window.location.assign("/account")} className="pressable grid place-items-center gap-0.5 p-1.5 text-[10px] font-bold text-[#536b8c]"><UserRound className="size-5" />Account</button><button onClick={openCart} className="pressable relative grid place-items-center gap-0.5 p-1.5 text-[10px] font-bold text-[#536b8c]"><ShoppingBag className="size-5" />Bag<span className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-[#f2683a] text-[8px] text-white">{itemCount}</span></button></nav>
