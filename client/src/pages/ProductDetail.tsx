...
function ImageZoomDialog({ product, selectedImageIndex, onClose, onChangeImage }: { product: Product; selectedImageIndex: number; onClose: () => void; onChangeImage: (direction: "previous" | "next") => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#061b3b]/90 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Expanded image for ${product.title}`} onClick={onClose}><section className="relative flex h-full w-full max-w-5xl flex-col rounded-[1.5rem] bg-[#fffdf9] p-3 shadow-2xl sm:p-5" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between gap-4 px-2 pb-3"><p className="line-clamp-1 text-xs font-extrabold text-[#0a285a]">{product.title}</p><button autoFocus onClick={onClose} aria-label="Close expanded product image" className="pressable grid size-10 place-items-center rounded-full border border-[#d5dfeb] bg-white text-[#0a285a] hover:bg-[#e7edf5]"><X className="size-4" /></button></div><div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-[1rem] bg-[#e7edf5]">{image ? <img src={image.url} alt={image.altText ?? product.title} className="max-h-full max-w-full object-contain" /> : null}{hasMultipleImages ? <><button onClick={() => onChangeImage("previous")} aria-label="Show previous product image" className="pressable absolute left-2 top-1/2 grid size-11 -translate-y-1/2 place-items-center text-white drop-shadow-[0_2px_2px_rgba(10,40,90,.55)] hover:bg-white/90 hover:text-[#f2683a] hover:drop-shadow-none"><ChevronLeft className="size-7" /></button><button onClick={() => onChangeImage("next")} aria-label="Show next product image" className="pressable absolute right-2 top-1/2 grid size-11 -translate-y-1/2 place-items-center text-white drop-shadow-[0_2px_2px_rgba(10,40,90,.55)] hover:bg-white/90 hover:text-[#f2683a] hover:drop-shadow-none"><ChevronRight className="size-7" /></button><span className="absolute bottom-3 right-3 rounded-full bg-[#0a285a]/85 px-3 py-1.5 text-[10px] font-extrabold text-white">{selectedImageIndex + 1} / {product.images.length}</span></> : null}</div></section></div>;
}
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-[#d5dfeb] bg-white">
      <div className="relative bg-[#e7edf5]">
        ...
        {image ? <button onClick={onZoom} aria-label="Expand product image" className="pressable absolute right-3 top-3 grid size-10 place-items-center rounded-full bg-white/90 text-[#0a285a] shadow-sm hover:bg-white hover:text-[#f2683a]"><Maximize2 className="size-4" /></button> : null}
        {hasMultipleImages ? <>
          <button
            onClick={() => onChangeImage("previous")}
            aria-label="Show previous product image"
            className="pressable absolute left-2 top-1/2 grid size-11 -translate-y-1/2 place-items-center text-white drop-shadow-[0_2px_2px_rgba(10,40,90,.55)] transition-colors hover:bg-white/90 hover:text-[#f2683a] hover:drop-shadow-none focus-visible:bg-white/90 focus-visible:text-[#f2683a] focus-visible:outline-none"
          >
          ...
          <button
            ...
            className="pressable absolute right-2 top-1/2 grid size-11 -translate-y-1/2 place-items-center text-white drop-shadow-[0_2px_2px_rgba(10,40,90,.55)] transition-colors hover:bg-white/90 hover:text-[#f2683a] hover:drop-shadow-none focus-visible:bg-white/90 focus-visible:text-[#f2683a] focus-visible:outline-none"
          >
            <ChevronRight className="size-7" strokeWidth={2.35} />
          </button>
          <span className="absolute bottom-3 right-3 rounded-full bg-[#0a285a]/85 px-3 py-1.5 text-[10px] font-extrabold text-white backdrop-blur">{selectedImageIndex + 1} / {product.images.length}</span>
        </> : null}
      </div>
      ...
      <div className="container py-6 sm:py-9">
        ...
        <section className="mt-5 grid gap-7 lg:grid-cols-[minmax(0,1.06fr)_minmax(350px,.94fr)] lg:gap-12">
          ...
          <div className="flex flex-col rounded-[2rem] border border-[#d5dfeb] bg-[#fffdf9] p-6 sm:p-9">
            <div className="flex items-start justify-between gap-4"><span className="rounded-full bg-[#fff0ab] px-3 py-1.5 text-[10px] font-extrabold tracking-[.13em] text-[#0a285a] uppercase">{product.productType || "NetLet edit"}</span><div className="flex gap-2"><button onClick={() => toggleSaved(product.id)} aria-label={isSaved ? `Remove ${product.title} from saved` : `Save ${product.title}`} className={`pressable grid size-10 place-items-center rounded-full border border-[#d5dfeb] bg-white ${isSaved ? "text-[#f2683a]" : "text-[#0a285a]"}`}><Heart className={`size-4 ${isSaved ? "fill-current" : ""}`} /></button><button onClick={() => void shareProduct(product)} aria-label={`Share ${product.title}`} className="pressable grid size-10 place-items-center rounded-full border border-[#d5dfeb] bg-white text-[#0a285a]"><Share2 className="size-4" /></button></div></div>
            ...
            <button onClick={() => void addToBag()} disabled={!selectedVariant?.availableForSale || cartLoading} className="pressable mt-7 flex w-full items-center justify-center gap-2 rounded-full bg-[#0a285a] px-5 py-4 text-sm font-extrabold text-white hover:bg-[#f2683a] disabled:cursor-not-allowed disabled:opacity-50">{cartLoading ? <LoaderCircle className="size-4 animate-spin" /> : <ShoppingBag className="size-4" />}{selectedVariant?.availableForSale ? "Add to bag" : "Unavailable"}</button>
