...
type RawMetafield = { namespace: string; key: string; value: string | null };
...
function normalizeAttribute(metafield: RawMetafield): ProductAttribute | null {
  if (!metafield.value?.trim()) return null;
  return { namespace: metafield.namespace, key: metafield.key, label: attributeLabel(metafield.key), value: metafield.value.trim() };
}
...
export function normalizeProduct(p: RawProduct): Product {
  return {
    ...
    tags: p.tags ?? [],
    attributes: (p.metafields ?? []).flatMap((metafield) => metafield ? [normalizeAttribute(metafield)].filter((attribute): attribute is ProductAttribute => attribute !== null) : []),
